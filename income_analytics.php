...
<main>
  <div class="header">
    ...
    <p>Detailed analysis of your income sources and trends</p>
  </div>

  <div class="stats-grid">
    <div class="stat-card">
      <h3>Total Income</h3>
      <div class="value"><?= format_money(array_sum(array_column($income_rows, 'amount'))); ?></div>
      <div>All time</div>
    </div>
    <div class="stat-card">
      <h3>Average Monthly Income</h3>
      <div class="value"><?= format_money(array_sum(array_column($income_rows, 'amount')) / max(1, count(array_unique(array_column($income_rows, 'date'))))); ?></div>
      <div>Based on <?= count($income_rows); ?> entries</div>
    </div>
    <div class="stat-card">
      <h3>Income Sources</h3>
      <div class="value" style="color: #f39c12;"><?= count(array_unique(array_column($income_rows, 'source'))); ?></div>
      <div>Different sources</div>
    </div>
    <div class="stat-card">
      <h3>Largest Source</h3>
      <div class="value" style="color: #9b59b6;"><?= !empty($income_sources) ? array_keys($income_sources, max($income_sources))[0] : 'N/A'; ?></div>
      <div>By amount</div>
    </div>
  </div>
